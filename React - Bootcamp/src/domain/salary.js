...
function calculateSalaryFrom(fullSalary) {
  ...
  const percentageInss = baseINSS ? (discountINSS / baseINSS) * 100 : 0;
  const percentageIrpf = baseINSS ? (discountIRPF / baseINSS) * 100 : 0;
  const percentageNetSalary = baseINSS ? (netSalary / baseINSS) * 100 : 0;
